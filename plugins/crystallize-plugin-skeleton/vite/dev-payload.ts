...
// Vite's plugin runtime can be Node even when started via bun, so we don't rely on Bun.JSON5.
...
// This Vite plugin runs in Vite's Node process, which is started by `vite`
// (not `node --env-file`), so `.env` is NOT auto-loaded into `process.env`
// here. Parse it ourselves so populate-mode env overrides
...
// signal — to both the server's payload-decrypter and the crystallize client
// builder — that this envelope was minted locally. Dev API credentials
// (accessTokenId/accessTokenSecret from `.env`) ride along inside the
...
const mintPayload = async (
    ...
    // Mirror that here so the server reads them off `decoded.envelope.*`
    ...
    // envelopeClaims even when signature verification fails. The server's
...
export function devPayloadPlugin(): Plugin {
    return {
        ...
        configureServer(server) {
            ...
            const devVarsPath = path.resolve(root, '.env');

            // Re-read `.env` per request so editing it doesn't require a
            ...
            server.middlewares.use(async (req, res, next) => {
                ...
                if (queryPayload) {
                    ...
                    // in `.env` instead of typing it into every URL or config file.
                    // Override `:tenant` for both the minted envelope AND the proxied
                    // sub-request URL — the server's payload-decrypter rejects mismatches
                    ...
                    // source them from `.env` so the server reads them off
                    ...
                    const pluginIdentifier = readEnv('PLUGIN_IDENTIFIER') ?? 'com.example.plugin.skeleton';
                    try {
